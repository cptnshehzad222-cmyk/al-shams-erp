...
function Expenses() {
  ...
  const formVatAmount = form.vat_applicable
    ? expenseAmount * (formVatRate / 100)
    : 0;
  ...
  async function saveExpense() {
    ...
    const vatAmount = form.vat_applicable
      ? expenseAmount *
        (Number(form.vat_rate) / 100)
      : 0;

    const totalAmount =
      expenseAmount + vatAmount;
    ...
    const expenseData = {
      ...
      person_vendor:
        form.person_vendor.trim() || null,
      amount: expenseAmount,
      vat_applicable: form.vat_applicable,
      vat_rate: form.vat_applicable
  ...
  const filteredExpenses = useMemo(() => {
    ...
    return expenses.filter((expense) => {
      ...
      if (
        branchFilter &&
        expense.branch_id !== branchFilter
      ) {
        return false;
      }
  ...
  const totalVat =
    filteredExpenses.reduce(
      (sum, expense) =>
        sum + Number(expense.vat_amount || 0),
      0
    );
  ...
  async function loadArabicFont(doc: jsPDF): Promise<boolean> {
    try {
      const fontPath = "/fonts/NotoSansArabic-Regular.ttf";

      const response = await fetch(fontPath);

      if (!response.ok) {
        throw new Error(
          `Arabic font could not be loaded. HTTP ${response.status}`
        );
      }
      ...
      if (!arrayBuffer || arrayBuffer.byteLength === 0) {
        throw new Error("Arabic font file is empty.");
      }
  ...
  function createSafeTable(doc: jsPDF, options: any) {
  ...
  function addCategorySummaryPdf(
    doc: jsPDF,
    data: Expense[],
    arabicFontLoaded: boolean
  ) {
  ...
  /* =======================================================
     FINAL SUMMARY PAGE - COMPLETE FIXED VERSION
  ======================================================= */
  ...
  async function generateMonthlyPdf() {
    ...
    if (
      ...
      alert(
        "Please enter the month like 2026-08."
      );
      return;
    }
    ...
    try {
      ...
      if (
        ...
        alert(
          `No expenses found for ${formatMonth(
            month
          )}.`
        );
        return;
      ...
      doc.text(
        `Number of Expense Records: ${monthlyExpenses.length}`,
        18,
        143
      );

      doc.text(
        "Report Type: Full Monthly Expense Report",
        18,
        152
      );
  ...
  async function generateFilteredPdf() {
    ...
    try {
      ...
      drawPdfSectionTitle(
        doc,
        "ALL FILTERED EXPENSE TRANSACTIONS",
        48
      );
      ...
      doc.save(
        `Expense_Report_${safeName}.pdf`
      );
    } catch (error) {
      console.error(
        "FILTERED PDF GENERATION ERROR:",
        error
      );
      ...
      alert(
        `Unable to generate filtered PDF report.\n\nError: ${errorMessage}`
      );
    } finally {
      setPdfLoading(false);
    }
    ...
    <div
      style={{
        background: highlight
          ? "linear-gradient(135deg,#082f49,#172554)"
          : "#0b1220",
